      ...
      <div className="p-8 max-w-7xl mx-auto flex flex-col gap-6 text-left">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
          ...
          <button
            ...
            className="px-4 py-2 bg-[#161A22] border border-[#2A303C] hover:bg-[#1A1F29] rounded text-xs font-semibold text-slate-300 transition-colors flex-shrink-0"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-4 mb-2">
                        <p className="text-xs text-slate-200 font-bold truncate flex-1 font-mono-code">
                          {item.question}
                        </p>
                        <span className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase tracking-wider font-mono-code border ${
                          item.status === 'success'
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-[9px] font-mono-code text-slate-500 border-t border-[#2A303C]/30 pt-2.5 mt-2">
                        <div>
                          Scope: <span className="text-slate-400 font-semibold">{item.mode === 'generate' ? 'f1_generate_only' : 'f2_execute'}</span>
                        </div>
                        <div>
                          Format: <span className="text-slate-400 font-semibold">{getFormatFromQuestion(item.question)}</span>
                        </div>
                        <div>
          ...
          {selectedItem && (
            <div className="lg:col-span-6 bg-[#161A22] border border-[#2A303C] rounded-[10px] shadow-lg overflow-hidden flex flex-col">
              <div className="bg-[#1A1F29] border-b border-[#2A303C] px-4 py-2 flex items-center justify-between font-mono-code text-[11px]">
              ...
              <div className="p-6 text-left font-mono-code text-xs space-y-4 max-h-[calc(100vh-280px)] overflow-y-auto">
                <div>
                ...
                <div className="border-t border-[#2A303C]/40 pt-3">
                  ...
                  <pre className="bg-[#0E1116] border border-[#2A303C] rounded p-4 overflow-x-auto text-green-400 font-mono-code leading-relaxed">
                ...
                <div className="grid grid-cols-2 gap-4 border-t border-[#2A303C]/40 pt-3">
                  <div>
                    ...
                    <span className="text-[#8B7CF6] font-bold">{selectedItem.mode === 'generate' ? 'Generate SQL Only' : 'Generate & Execute'}</span>
                  </div>
                  <div>
                    <span className="font-sans-ui font-semibold text-[9px] uppercase tracking-wider text-slate-500 block mb-0.5">Status</span>
                    <span className={`font-bold ${selectedItem.status === 'success' ? 'text-[#3ECF8E]' : 'text-[#EC5F5B]'}`}>{selectedItem.status.toUpperCase()}</span>
                  </div>
                  <div>
                    <span className="font-sans-ui font-semibold text-[9px] uppercase tracking-wider text-slate-500 block mb-0.5">Executed At</span>
                    <span className="text-slate-300">{new Date(selectedItem.created_at).toLocaleString()}</span>
                  </div>
                  <div>
                    <span className="font-sans-ui font-semibold text-[9px] uppercase tracking-wider text-slate-500 block mb-0.5">Format</span>
                    <span className="text-slate-300 capitalize">{getFormatFromQuestion(selectedItem.question)}</span>
                  </div>
                </div>

                {selectedItem.error_message && (
                  <div className="border-t border-[#2A303C]/40 pt-3">
                    <span className="font-sans-ui font-semibold text-[9px] uppercase tracking-wider text-red-400 block mb-1">Execution Error Log</span>
